 ...
 */
	...
	*/
	conjunto (const conjunto & d){
		this->value_type=d.value_type;
		this->size_type=d.size_type;
		this->iterator=d.iterator;
		this->const_iterator=d.const_iterator;
	}
	...
	*/
	pair<conjunto::value_type,bool>  find (const string & chr, const unsigned int & pos) const{
		std::pair<conjunto::value_type,bool> p1;
		bool encontrado=false;
		for(int i=0; i<=vm.end();i++){
			if(p1.first==(vm.begin()+i)){
				encontrado=true;
				break
			}
		}
		p1.second=encontrado;
		return p1;
	}
